...
const SkinsPage = () => {
  ...
  const [missingIds, setMissingIds] = useState<{ skins: Set<number>; capes: Set<number> } | null>(null);
  const [validating, setValidating] = useState(false);
  const [purging, setPurging] = useState(false);
  const serverUrl = getServerUrl();

  const fetchSkins = useCallback(
    (search: string, page: number) => skinsApi.listSkins({ page, pageSize: PAGE_SIZE, search }),
    [],
  );
  const fetchCapes = useCallback(
    (search: string, page: number) => skinsApi.listCapes({ page, pageSize: PAGE_SIZE, search }),
    [],
  );
  ...
  const handleValidate = useCallback(async () => {
    setValidating(true);
    try {
      const result = await skinsApi.validate();
  ...
  const handlePurgeMissing = useCallback(async () => {
    setPurging(true);
    try {
      const result = await skinsApi.purgeMissing();
      setMissingIds(null);
    ...
    <Main>
      ...
      <Box style={{ padding: `40px ${H_PAD} 32px` }}>
        <Flex justifyContent="space-between" alignItems="flex-start">
          <Box>
            <Typography variant="alpha">{translate('page.title')}</Typography>
            <Box paddingTop={1}>
      ...
      {activeTab === 'skins' && (
        <AssetTab
          ...
          onDelete={(id) => skinsApi.deleteSkin(id).then(() => {})}
        />
      )}
      ...
      {activeTab === 'capes' && (
        <AssetTab
          ...
          onDelete={(id) => skinsApi.deleteCape(id).then(() => {})}
        />
      )}
