...
const UploadModal = ({ onSkinUploaded, onCapeUploaded, onClose }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await Promise.all([
        skinFile
          ? skinsApi.uploadSkin(numericUserId, username || undefined, skinFile).then(onSkinUploaded)
          : Promise.resolve(),
        capeFile
          ? skinsApi.uploadCape(numericUserId, username || undefined, capeFile).then(onCapeUploaded)
          : Promise.resolve(),
      ]);
      onClose();
    } catch (err: unknown) {
