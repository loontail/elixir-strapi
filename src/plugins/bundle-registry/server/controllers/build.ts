...
const buildController = ({ strapi }: { strapi: Strapi }) => ({
  async find(ctx: KoaContext) {
    const builds: Build[] = await strapi.plugin('bundle-registry').service('build').findMany();
    ctx.body = builds;
  },
  ...
  async create(ctx: KoaContext) {
    ...
    const build: Build = await strapi.plugin('bundle-registry').service('build').create({
      name,
      slug,
      description,
      version,
  ...
  async delete(ctx: KoaContext) {
    ...
    await strapi.plugin('bundle-registry').service('build').deleteFileEntries(build.id);
    deleteBuildFiles(slug);
    await strapi.plugin('bundle-registry').service('build').delete(build.id);
  ...
  async uploadArchive(ctx: KoaContext) {
    const { slug } = ctx.params as { slug: string };
    const buildService = strapi.plugin('bundle-registry').service('build');
