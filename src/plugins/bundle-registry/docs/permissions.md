...
All admin routes (`/bundle-registry/builds/…`) are registered as **type: admin** routes with empty `policies: []`. They are protected automatically by Strapi's built-in admin JWT authentication.
...
The manifest endpoint (`GET /api/bundle-registry/builds/:slug/manifest`) is registered as a **content-api** route with `auth: false`, making it publicly accessible without any authentication. This is intentional — a launcher client needs to fetch the manifest without a session.
