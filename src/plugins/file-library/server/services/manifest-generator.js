...
async function generate(buildSlug, strapi) {
  ...
  for (const entry of fileEntries) {
    ...
    if (!entry.isDir) {
      ...
      fileData.url = storage.getPublicUrl(buildSlug, entry.relativePath, strapi);
      filesCount++;
  ...
  storage.writeManifestAtomic(buildSlug, grouped);

  const builds = await strapi.db.query('plugin::file-library.file-build').findMany({
